...
export default function App() {
  ...
  const navigate = (id: string) => { setCurrentId(id); setSelected(undefined); setQuery(""); };
  ...
  return (
    <div className="flex h-screen min-h-[640px] overflow-hidden bg-muted/30">
      <Sidebar root={fileTree} currentId={currentId} onNavigate={navigate} />
      <main className="flex min-w-0 flex-1 flex-col">
        ...
        <footer className="flex h-10 shrink-0 items-center justify-between border-t bg-background px-6 text-xs text-muted-foreground"><span>{selected ? `${selected.name} selected` : `${files.length} items`}</span><span>All files are up to date</span></footer>
      </main>
    </div>
  );
}
